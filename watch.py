...
async def main() -> None:
    ...
    confparse = configargparse.ArgParser(default_config_files=['./kwikset.conf', '/etc/kwikset.conf', '~/.kwikset.conf'])
    ...
    async with aiohttp.ClientSession() as http:
        while(True):
            ...
            # pprint(device_info)
            logging.info(f"{device_info['batterypercentage']}% {device_info['batterystatus']}")
            if conf.heartbeaturl:
                await http.get('https://heartbeat.uptimerobot.com/m798367254-c8f5713329ac5a83ce29a23a97af65c20a902e6e')   
